function move_sensors_gradient(sensors,target_loc,plant_measurement)
  ...
  num_sensors =length(sensors);
   ...
   for k=1:num_sensors
      ...
      dbdr=barrier_derivative(r(k));
      gradient=(dLdr+dbdr)*r_hat(:,k)+(dLdeta/r(k))*t_hat(:,k);
      sensors(k).moveSensorGradient(gradient,descent_step_size); 
   end
    function dbdr=barrier_derivative(r)
         global Min_Separation;
           global mu;
           global barrier_order;
         dbdr=(2*barrier_order+1)*mu/((r-Min_Separation)^(2*barrier_order+2));
    end
    function [dLdeta,dLdr]=partial_derivatives(h,r_hat,k,num_sensors)
        % With barrier included
        global b;
